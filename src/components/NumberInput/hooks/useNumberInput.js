...
export default ({
  ...
  const handleChange = useCallback(
    ...
    [onChange, countOfDigitsAfterPoint, thousandsSeparator, onlyValue, max, min]
  );
  ...
  const handleKeyDown = useCallback(
    (e) => {
      ...
      const shouldCaptureAdditionalSymbols =
        selectionEnd - selectionStart === 0;
      ...
      if (shouldCaptureAdditionalSymbols) {
        if (isForward) {
          if (keyCode === 46) {
            ...
            selectionStart++;
            while (
              thousandsSeparator.includes(inputValue[selectionStart - 1])
            ) {
              selectionStart++;
            }
          }
    ...
    [onChange, thousandsSeparator, countOfDigitsAfterPoint, onlyValue]
  );
